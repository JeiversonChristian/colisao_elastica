...
window.onload = function() {
    ...
    let c1v = 0; // velocidade da esfera que se atualiza
    ...
    function mostrar_informacoes(){
        ...
        if (c1_colidiu == false) {
            ctx.fillText(`v1f: "A Definir"`,50,window.canvas.height-7);
        }
        else{
            ctx.fillText(`v1f ≅ ${c1v.toFixed(2)} m/s`,50,window.canvas.height-7);
        }
        // Esfera 2
        ctx.textAlign = "right";
        ...
        if (c1_colidiu == false) {
            ctx.fillText(`v1f: "A Definir"`,window.canvas.width-60,window.canvas.height-7);
        }
        else{
    ...
    function verificar_botoes(){
        ...
        // se apertar o botão, aumenta ou diminui o valor (massa/velocidade)
        // massa
